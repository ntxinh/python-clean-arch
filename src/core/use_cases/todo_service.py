...
class TodoService:
    ...
    async def get_todos(self, page: int = 1, size: int = 10, search: Optional[str] = None) -> List[Todo]:
        offset = (page - 1) * size
        return await self.repo.list(limit=size, offset=offset, title_search=search)

    async def update_todo(self, todo_id: int, title: Optional[str], is_completed: Optional[bool]) -> Optional[Todo]:
